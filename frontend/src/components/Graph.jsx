...
const Graph = () => {
    ...
    // Проверка: R должен быть числом и входить в диапазон [1 ... 4]
    const isRValid = !isNaN(currentR) && currentR >= 1 && currentR <= 4;

    const handleClick = (e) => {
        // Блокируем клик, если R невалиден
        if (!isRValid) {
    ...
    return (
        <svg
            ...
            style={{
                backgroundColor: '#fff',
                borderRadius: '12px',
                cursor: isRValid ? 'crosshair' : 'not-allowed', // Меняем курсор, если R не ок
        ...
        >
            ...
            {/* --- Точки (Рисуем ТОЛЬКО если R валиден) --- */}
